...
public interface SurveyRepository extends JpaRepository<Survey, Long> {
    ...
    @Query("SELECT s FROM Survey s LEFT JOIN s.surveyResults sr WHERE s.id IN " +
            "(SELECT DISTINCT sr2.survey.id FROM SurveyResult sr2 WHERE sr2.user.id = :userId) " +
            "ORDER BY s.createdAt DESC")
    List<Survey> findJoinedSurveysOrderByNewest(@Param("userId") long userId);

    @Query("SELECT s FROM Survey s LEFT JOIN s.surveyResults sr WHERE s.id IN " +
            "(SELECT DISTINCT sr2.survey.id FROM SurveyResult sr2 WHERE sr2.user.id = :userId) " +
            "ORDER BY s.createdAt ASC")
    List<Survey> findJoinedSurveysOrderByOldest(@Param("userId") long userId);

    @Query("SELECT s FROM Survey s LEFT JOIN s.surveyResults sr WHERE s.id IN " +
            "(SELECT DISTINCT sr2.survey.id FROM SurveyResult sr2 WHERE sr2.user.id = :userId) " +
            "GROUP BY s.id ORDER BY COUNT(sr.id) DESC")
    List<Survey> findJoinedSurveysOrderByParticipantCountDesc(@Param("userId") long userId);
}
